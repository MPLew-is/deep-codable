...
/// Test encoding types containing only "bare" values (no `@Value` wrappers).
final class BareEncodingTests: XCTestCase {
	...
	func testBranchedTopLevelEncoding() throws {
		struct BranchedTopLevelEncoding: DeepEncodable {
			static let codingTree = CodingTree {
				Key("top1", containing: \.key1)
				Key("top2", containing: \.key2)
			}
	...
	func testBranchedSecondLevelEncoding() throws {
		struct BranchedSecondLevelEncoding: DeepEncodable {
			static let codingTree = CodingTree {
				Key("top1") {
					Key("second1", containing: \.key1)
				}

				Key("top2") {
					Key("second2", containing: \.key2)
				}
			}
	...
	func testFlattenedExcessivelyDeepEncoding() throws {
		struct FlattenedExcessivelyDeepEncoding: DeepEncodable {
			static let codingTree = CodingTree {
				Key("top", "second", "third", "fourth", "fifth", "sixth", "seventh", "eighth", "ninth", "tenth", containing: \.key)
			}
		...
		struct FlattenedBranchedExcessivelyDeepEncoding: DeepEncodable {
			static let codingTree = CodingTree {
				Key("top", "second", "third", "fourth", "fifth") {
					Key("sixth1", "seventh1", "eighth1", "ninth1", "tenth1", containing: \.key1)
					Key("sixth2", "seventh2", "eighth2", "ninth2", "tenth2", containing: \.key2)
				}
			}
	...
	func testOptionalValuesEncodeEmpty() throws {
		struct OptionalValuesEncodeEmpty: DeepEncodable {
			static let codingTree = CodingTree {
				Key("top") {
					Key("second1") {
						Key("third1", containing: \.key1)
					}

					Key("second2") {
						Key("third2") {
							Key("fourth2", containing: \.key2)
						}
					}
				}
			}
	...
	*/
	func testOptionalEncodingSameInstanceNilThenValue() throws {
		...
		var instance = OptionalEncodingSameInstanceNilThenValue(key: nil)
	...
	*/
	func testOptionalEncodingSameInstanceValueThenNil() throws {
		...
		var instance = OptionalEncodingSameInstanceValueThenNil(key: "secondValue")
